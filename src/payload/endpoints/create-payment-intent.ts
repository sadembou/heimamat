...
export const createPaymentIntent: PayloadHandler = async (req, res): Promise<void> => {
  ...
  try {
    ...
    for (const item of cartItems) {
      ...
      payload.logger.info(`Total amount for ${product.title} is ${prices.data[0].unit_amount*quantity}`)
    ...
    const send_invoice = await stripe.invoices.sendInvoice(invoice.id)
    res.status(200).json({
      message: 'Invoice created and sent successfully',
      invoice: {
        id: invoice.id,
        url: send_invoice.hosted_invoice_url,
      },
    });
...
/*export const createPaymentIntent_old: PayloadHandler = async (req, res): Promise<void> => {
  const { user, payload } = req
